...
/*invariant: maxRoute(i,j) always returns the maximum length from (i,j) to the bottom*/ 
int maxRoute(int level, int index_on_level)
{
	...
	if(index_on_level>0 && triangle[index-1] >= triangle[index])
		return triangle[index]+maxRoute(level+1,index_on_level+1); // wrong because the invariant is broken here!
